    ...
    */
    ...
    public async Task<List<VoiceInfo>> GetAvailableVoicesAsync(TextToSpeechServiceProviderConfig config, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                voices = _openAiVoiceNames.Select(v => new VoiceInfo { ShortName = v.ToString() }).ToList();
                return voices;
            }
            else if (config == TextToSpeechServiceProviderConfig.AzureSpeechService)
            {
                ...
                if (voicesResult.Reason == ResultReason.VoicesListRetrieved)
                {
                    voices = voicesResult.Voices
                        ...
                        .ToList();
                    _logger.LogDebug($"Retrieved {voices.Count} voices.");
                }
                else
                {
                ...
                // Cache the retrieved voices and provider
