...
{
    private const string TerminatingResponseMarker = "[COMPLETE]";
    ...
    {       
        ...
        {            
            ...
            // Check for TerminatingResponseMarker and strip it out
            var completeMarker = agentMessage.LastIndexOf(TerminatingResponseMarker, StringComparison.OrdinalIgnoreCase);
            if (completeMarker != -1)                
                agentMessage = agentMessage.Remove(completeMarker, TerminatingResponseMarker.Length);
            ...
            // If agent response sounds like a terminating response, complete the conversation.
            if (completeMarker != -1)
                return; // Complete the conversation loop
        }
    }
